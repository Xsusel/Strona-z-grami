...
function handleAction(io, socket, room, action, data) {
    switch (action) {
        case 'draw':
            handleDraw(io, room, data.drawData);
            break;
        case 'next-turn':
            handleNextTurn(io, socket, room);
            break;
        case 'vote-continue':
...
function handleDraw(io, room, drawData) {
    room.gameState.canvasState.push(drawData);
    io.to(room.name).emit('update-canvas', drawData);
}

function handleNextTurn(io, socket, room) {
    room.gameState.turn++;
    const players = Object.keys(room.players);
